...
function RedDivider({ label }) {
  return (
    <div className="flex items-center justify-center gap-4 mb-5">
      <div className="h-px w-10 bg-gradient-to-r from-transparent to-[#C41E3A]" />
      <span className="text-[#C41E3A] text-[10px] tracking-[0.35em] uppercase font-medium">{label}</span>
      <div className="h-px w-10 bg-gradient-to-l from-transparent to-[#C41E3A]" />
    </div>
  )
}
  ...
  return (
    <motion.nav
      ...
      className={`fixed top-0 left-0 right-0 z-50 transition-all duration-500 ${
        scrolled ? 'bg-[#1a1a1a]/95 backdrop-blur-md shadow-xl shadow-black/60 border-b border-[#C41E3A]/10' : 'bg-transparent'
      }`}
    >
        ...
        <div className="flex items-center justify-between h-16 lg:h-20">
          <a href="#accueil" className="flex items-center gap-3 group">
            <div className="relative w-9 h-9">
              <div className="absolute inset-0 border-2 border-[#C41E3A] rotate-45 group-hover:rotate-[55deg] transition-transform duration-500" />
              <div className="absolute inset-0 flex items-center justify-center">
                <span className="text-[#C41E3A] text-[10px] font-bold tracking-tight font-serif">BS</span>
              </div>
            </div>
            <div className="leading-none">
              <p className="font-serif text-[#F5F0E8] text-sm font-semibold tracking-[0.15em] uppercase">Barber Shop</p>
              <p className="text-[#C41E3A] text-[10px] tracking-[0.3em] uppercase">de La Roquette</p>
            </div>
          </a>
          <div className="hidden md:flex items-center gap-6 lg:gap-8">
            {navLinks.map(({ label, href }) => (
              <a key={label} href={href} className="relative text-[#F5F0E8]/60 hover:text-[#C41E3A] text-xs tracking-[0.2em] uppercase font-medium transition-colors duration-300 group py-1">
                {label}
                <span className="absolute -bottom-0.5 left-0 h-px w-0 bg-[#C41E3A] group-hover:w-full transition-all duration-300" />
              </a>
            ))}
            <a href="tel:+33682367193" className="ml-2 bg-[#C41E3A] hover:bg-[#E53935] text-white text-xs font-bold tracking-[0.2em] uppercase px-5 py-2.5 transition-all duration-300 hover:shadow-[0_0_20px_rgba(196,30,58,0.5)]">
              Réserver
            </a>
          </div>
      ...
      <AnimatePresence>
        {menuOpen && (
          <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }} transition={{ duration: 0.3 }}
            className="md:hidden overflow-hidden bg-[#141414]/98 backdrop-blur-md border-t border-[#C41E3A]/15">
            <div className="px-5 py-5 flex flex-col gap-1">
              {navLinks.map(({ label, href }) => (
                <a key={label} href={href} onClick={() => setMenuOpen(false)}
                  className="text-[#F5F0E8]/70 hover:text-[#C41E3A] text-sm tracking-[0.2em] uppercase py-3 border-b border-[#2a2a2a] transition-colors">
                  {label}
                </a>
              ))}
              <a href="tel:+33682367193" onClick={() => setMenuOpen(false)}
                className="mt-4 bg-[#C41E3A] hover:bg-[#E53935] text-white text-sm font-bold tracking-[0.2em] uppercase text-center py-4 transition-colors">
...
function Hero() {
  return (
    <section id="accueil" className="relative min-h-screen flex items-center justify-center overflow-hidden">
      ...
      <div className="absolute left-6 xl:left-12 top-0 bottom-0 w-px bg-gradient-to-b from-transparent via-[#C41E3A]/25 to-transparent hidden lg:block" />
      <div className="absolute right-6 xl:right-12 top-0 bottom-0 w-px bg-gradient-to-b from-transparent via-[#C41E3A]/25 to-transparent hidden lg:block" />
      <div className="relative z-10 text-center px-5 max-w-5xl mx-auto">
        <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8, delay: 0.2 }}
          ...
          <div className="h-px w-14 bg-[#C41E3A]" />
          <span className="text-[#C41E3A] text-[11px] tracking-[0.4em] uppercase font-medium">Arles · Ouvert 7j/7</span>
          <div className="h-px w-14 bg-[#C41E3A]" />
        </motion.div>
        <motion.h1 initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.9, delay: 0.4 }}
          ...
          <span className="block text-[#C41E3A] italic text-5xl sm:text-7xl lg:text-[7rem] xl:text-[8rem]">Barbier</span>
        </motion.h1>
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8, delay: 0.85 }}
          ...
          <a href="tel:+33682367193" className="flex items-center gap-3 bg-[#C41E3A] hover:bg-[#E53935] text-white font-bold px-8 py-4 text-sm tracking-[0.2em] uppercase transition-all duration-300 hover:shadow-[0_0_40px_rgba(196,30,58,0.5)] group">
          ...
          <a href="#services" className="text-[#F5F0E8]/75 hover:text-[#C41E3A] text-sm tracking-[0.2em] uppercase border border-[#F5F0E8]/20 hover:border-[#C41E3A]/60 px-8 py-4 transition-all duration-300">
            Voir les Tarifs
          </a>
        </motion.div>
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 1, delay: 1.2 }}
          ...
          {[{ value: '4.7★', label: 'Note Google' }, { value: '37+', label: 'Avis clients' }, { value: '7j/7', label: '09h – 21h' }].map(({ value, label }) => (
            <div key={label} className="text-center">
              <p className="font-serif text-[#C41E3A] text-2xl font-bold leading-none">{value}</p>
      ...
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 1.8, duration: 0.8 }}
        ...
        <div className="w-5 h-8 rounded-full border border-[#C41E3A]/35 flex items-start justify-center pt-1.5">
          <motion.div animate={{ y: [0, 10, 0] }} transition={{ repeat: Infinity, duration: 1.6 }} className="w-1 h-2 bg-[#C41E3A] rounded-full" />
        </div>
      </motion.div>
    </section>
  )
...
function Services() {
  return (
    <section id="services" className="py-24 lg:py-32 bg-[#1a1a1a] relative overflow-hidden">
      <div className="absolute inset-0 opacity-[0.04]" style={{ backgroundImage: 'radial-gradient(circle, #C41E3A 1px, transparent 1px)', backgroundSize: '32px 32px' }} />
      <div className="absolute top-0 left-0 w-24 h-24 border-l border-t border-[#C41E3A]/20" />
      <div className="absolute bottom-0 right-0 w-24 h-24 border-r border-b border-[#C41E3A]/20" />
      <div className="relative max-w-7xl mx-auto px-5 sm:px-8 lg:px-12">
        <motion.div variants={stagger} initial="hidden" whileInView="visible" viewport={{ once: true, margin: '-80px' }} className="text-center mb-16">
          <motion.div variants={fadeUp}><RedDivider label="Nos Prestations" /></motion.div>
          ...
          {SERVICES.map((s) => (
            <motion.div key={s.name} variants={fadeUp}
              className={`relative group p-7 lg:p-8 border transition-all duration-500 cursor-default overflow-hidden ${
                s.featured ? 'bg-gradient-to-br from-[#C41E3A]/15 via-[#C41E3A]/8 to-transparent border-[#C41E3A]/50 hover:border-[#C41E3A]'
                  : 'bg-[#242424] border-[#333] hover:border-[#C41E3A]/35 hover:bg-[#272727]'
              }`}>
              {s.featured && <div className="absolute top-0 right-0"><div className="bg-[#C41E3A] text-white text-[9px] font-bold px-3 py-1.5 tracking-[0.25em] uppercase">Populaire</div></div>}
              ...
              <div className="flex items-start justify-between mb-5">
                ...
                <span className="font-serif text-[#C41E3A] text-2xl lg:text-3xl font-bold">{s.price}</span>
              </div>
              ...
              <div className="flex items-center gap-2 text-[#F5F0E8]/25 text-xs">
                <svg className="w-3.5 h-3.5 text-[#C41E3A]/50" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
                <span>{s.duration}</span>
              </div>
              <div className="absolute bottom-0 left-0 h-0.5 w-0 bg-gradient-to-r from-[#C41E3A] to-[#E53935] group-hover:w-full transition-all duration-500" />
            </motion.div>
          ))}
        </motion.div>
        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: 0.5 }} className="text-center mt-12">
          <a href="tel:+33682367193" className="inline-flex items-center gap-3 text-[#C41E3A] border border-[#C41E3A]/30 hover:border-[#C41E3A] hover:bg-[#C41E3A]/8 px-8 py-4 text-xs tracking-[0.25em] uppercase font-medium transition-all duration-300">
  ...
  return (
    <div className="absolute inset-0 bg-gradient-to-t from-[#1a1a1a]/90 via-[#1a1a1a]/20 to-transparent opacity-0 group-hover:opacity-100 transition-all duration-500 flex items-end p-4">
      <div className="translate-y-3 group-hover:translate-y-0 transition-transform duration-300">
        ...
        <div className="h-px w-8 bg-[#C41E3A] mt-1.5" />
      </div>
    </div>
  )
}
...
function Gallery() {
  return (
    <section id="galerie" className="py-24 lg:py-32 bg-[#141414]">
      <div className="max-w-7xl mx-auto px-5 sm:px-8 lg:px-12">
        <motion.div variants={stagger} initial="hidden" whileInView="visible" viewport={{ once: true, margin: '-80px' }} className="text-center mb-16">
          <motion.div variants={fadeUp}><RedDivider label="Notre Univers" /></motion.div>
...
function Reviews() {
  return (
    <section id="avis" className="py-24 lg:py-32 bg-[#1a1a1a]">
      <div className="max-w-7xl mx-auto px-5 sm:px-8 lg:px-12">
        <motion.div variants={stagger} initial="hidden" whileInView="visible" viewport={{ once: true, margin: '-80px' }} className="text-center mb-16">
          <motion.div variants={fadeUp}><RedDivider label="Ils nous font confiance" /></motion.div>
          ...
          <motion.div variants={fadeUp} className="inline-flex flex-col items-center gap-4 bg-[#242424] border border-[#333] px-12 py-8 relative">
            <div className="absolute top-0 left-0 w-5 h-5 border-l-2 border-t-2 border-[#C41E3A] -translate-x-0.5 -translate-y-0.5" />
            <div className="absolute top-0 right-0 w-5 h-5 border-r-2 border-t-2 border-[#C41E3A] translate-x-0.5 -translate-y-0.5" />
            <div className="absolute bottom-0 left-0 w-5 h-5 border-l-2 border-b-2 border-[#C41E3A] -translate-x-0.5 translate-y-0.5" />
            <div className="absolute bottom-0 right-0 w-5 h-5 border-r-2 border-b-2 border-[#C41E3A] translate-x-0.5 translate-y-0.5" />
            <span className="font-serif text-[#C41E3A] text-7xl font-bold leading-none">4.7</span>
        ...
        <motion.div variants={stagger} initial="hidden" whileInView="visible" viewport={{ once: true, margin: '-80px' }} className="grid grid-cols-1 md:grid-cols-3 gap-5">
          {REVIEWS.map((r, i) => (
            <motion.div key={i} variants={fadeUp} className="bg-[#242424] border border-[#333] p-7 hover:border-[#C41E3A]/25 transition-all duration-300 group relative overflow-hidden">
              <div className="absolute top-4 right-5 font-serif text-6xl text-[#C41E3A]/8 leading-none select-none">&ldquo;</div>
              <div className="flex items-center gap-3 mb-4">
                <div className="w-11 h-11 bg-gradient-to-br from-[#C41E3A]/30 to-[#C41E3A]/10 border border-[#C41E3A]/30 flex items-center justify-center text-[#C41E3A] font-serif font-bold text-lg flex-shrink-0">{r.name[0]}</div>
                <div>
              ...
              <div className="absolute bottom-0 left-0 h-0.5 w-0 bg-[#C41E3A] group-hover:w-full transition-all duration-500" />
            </motion.div>
          ))}
        </motion.div>
      </div>
...
function Contact() {
  ...
  return (
    <section id="contact" className="py-24 lg:py-32 bg-[#141414] relative overflow-hidden">
      ...
      <div className="relative max-w-7xl mx-auto px-5 sm:px-8 lg:px-12">
        <motion.div variants={stagger} initial="hidden" whileInView="visible" viewport={{ once: true, margin: '-80px' }} className="text-center mb-16">
          <motion.div variants={fadeUp}><RedDivider label="Nous trouver" /></motion.div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-16 items-start">
          <motion.div variants={stagger} initial="hidden" whileInView="visible" viewport={{ once: true, margin: '-80px' }}>
            <motion.div variants={stagger} className="space-y-3 mb-10">
              {infoItems.map(({ icon, label, value, sub, link }) => (
                <motion.div key={label} variants={fadeUp}
                  className="flex items-start gap-4 p-5 bg-[#1a1a1a] border border-[#2e2e2e] hover:border-[#C41E3A]/30 transition-all duration-300 group">
                  <div className="w-11 h-11 border border-[#C41E3A]/25 flex items-center justify-center text-[#C41E3A] flex-shrink-0 group-hover:bg-[#C41E3A]/8 group-hover:border-[#C41E3A]/50 transition-all duration-300">{icon}</div>
                  <div>
                    <p className="text-[#F5F0E8]/35 text-[10px] tracking-[0.3em] uppercase mb-1">{label}</p>
                    {link ? <a href={link} className="text-[#F5F0E8] font-medium text-base hover:text-[#C41E3A] transition-colors">{value}</a>
            ...
            <motion.div variants={fadeUp}>
              <a href="tel:+33682367193" className="group flex items-center justify-center gap-4 w-full bg-[#C41E3A] hover:bg-[#E53935] text-white font-bold py-5 text-sm tracking-[0.25em] uppercase transition-all duration-300 hover:shadow-[0_0_50px_rgba(196,30,58,0.5)]">
                <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24"><path d="M6.6 10.8c1.4 2.8 3.8 5.1 6.6 6.6l2.2-2.2c.27-.27.67-.36 1.02-.24 1.12.37 2.33.57 3.57.57.55 0 1 .45 1 1V20c0 .55-.45 1-1 1C10.29 21 3 13.71 3 4.5c0-.55.45-1 1-1H7.5c.55 0 1 .45 1 1 0 1.25.2 2.45.57 3.57.11.35.03.74-.23 1.01L6.6 10.8z"/></svg>
          ...
          <motion.div initial={{ opacity: 0, x: 40 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true, margin: '-80px' }} transition={{ duration: 0.9 }} className="relative">
            <div className="absolute -inset-1 bg-gradient-to-br from-[#C41E3A]/15 via-transparent to-[#C41E3A]/8 blur-sm" />
            <div className="relative border border-[#C41E3A]/20 overflow-hidden" style={{ aspectRatio: '4/3' }}>
              <iframe title="Barber Shop de La Roquette"
              ...
              <div className="absolute top-4 left-4 bg-[#1a1a1a]/92 backdrop-blur-sm border border-[#C41E3A]/40 px-4 py-2.5 pointer-events-none">
                <p className="text-[#C41E3A] text-[10px] font-bold tracking-[0.2em] uppercase">Barber Shop de La Roquette</p>
                <p className="text-[#F5F0E8]/55 text-xs mt-0.5">70 Bd Georges Clemenceau, 13200 Arles</p>
              </div>
              <div className="absolute top-0 right-0 w-8 h-8 border-r-2 border-t-2 border-[#C41E3A]/50 pointer-events-none" />
              <div className="absolute bottom-0 left-0 w-8 h-8 border-l-2 border-b-2 border-[#C41E3A]/50 pointer-events-none" />
            </div>
          </motion.div>
        </div>
      </div>
        ...
        <div className="flex flex-col md:flex-row items-center justify-between gap-6">
          <div className="flex items-center gap-3">
            <div className="relative w-7 h-7">
              <div className="absolute inset-0 border border-[#C41E3A]/60 rotate-45" />
              <div className="absolute inset-0 flex items-center justify-center">
                <span className="text-[#C41E3A] text-[8px] font-bold font-serif">BS</span>
              </div>
            </div>
            <div className="leading-none">
          ...
          <a href="tel:+33682367193" className="text-[#C41E3A] text-sm hover:text-[#E53935] transition-colors duration-300 tracking-wider">06 82 36 71 93</a>
        </div>
      </div>
    </footer>
  )
